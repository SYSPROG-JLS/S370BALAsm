...
def handle_IIBDDD(operands, base_reg):
    ...
    if II.startswith('C'):
        ...
        Idata = II[2:4]
    elif II.startswith('B'):
        Idata = hex(int(II[2:10],2)).lstrip('0x').rjust(2,'0').upper()
    elif II.isnumeric():
        Idata = hex(int(II)).lstrip('0x').rjust(2,'0').upper()
...
def handle_R0BDDD(operands):
    ...
    BDDD = BDDD.rstrip('(0)')      #handle case where you have ,32(0)
...
def handle_RXBDDD(operands, base_reg):
    if operands.count(',') == 1:
        ...
        if R1[0].isalpha():
            R1 = symbol_table_dict[R1]
        R1data = hex(int(R1))[2:].upper()
        if oper2[0].isalpha():                             #STC   R4,AREA1 / STC   R4,AREA1+1 / R4,X'FF'
            if oper2.count("'") == 2:                      #R4,C'0'  R4,X'F0'   R4,B'11110000'
                X2 = '0'
                B2 = '0'
                if oper2.startswith('C'):
                    D2 = ASC2EBC[ord(oper2[2])].rjust(3,'0')
                elif oper2.startswith('X'):
                    D2 = oper2[2:4].rjust(3,'0')
                elif oper2.startswith('B'):
                    D2 = hex(int(oper2[2:10],2)).lstrip('0x').rjust(3,'0').upper()
            else:        
...
def parse_BDDD(BDDD, base_reg):
    if BDDD[0].isalpha():
        ...
        if isinstance(symbol_table_dict[BDDD],tuple):
            (addr, num_bytes) = symbol_table_dict[BDDD]
        else:
            addr = symbol_table_dict[BDDD]
...
def my_except_hook(exctype, value, traceback):
    if exctype == KeyError:
        print(f'Error at Line Number: {sl_ctr+1} - Symbol: {value} not found')
        print(f'Source Code: {source_code_list[sl_ctr]}')
    elif exctype == ValueError or exctype == UnboundLocalError or exctype == TypeError:
        print(f'Error at Line number: {sl_ctr+1} - General Syntax Error')
        print(f'Source Code: {source_code_list[sl_ctr]}')
    else:
...
#open the source code file and create a list 
#and a dictionary from assembler statements
ctr = 1
source_code_list = []
original_source_code_dict = {}
...
while True:
    sl = source_code_list[sl_ctr][4:]
    sl_t = source_code_list[sl_ctr][0:4]
    ...
    if mnemonic in mvs_macros_list or mnemonic == 'END':
        if mnemonic == 'DCB':
            st = []
            eodad = ''
            ddname = ''
            rwbyte = '0100'
            source_code_list[sl_ctr] = sl_t + '*' + sl
            if sl[0] != ' ':
            ...
            for s in st:
                sl_ctr += 1
                k = 'I'+str(i_ctr).rjust(3,'0')
                source_code_list.insert(sl_ctr, k+s) 
                original_source_code_dict[k] = s
                i_ctr += 1
            sl_ctr += 1
            continue                    
        elif mnemonic == 'OPEN':
...
while True:
    ...
    if '=' in t:
        (oper1, lit) = t.split('=')
        got_lit = False
        for lit_name, lit_value in lit_dict.items():
            if lit_value == lit:
                source_code_list[sl_ctr] = sl_t + sl[0:15] + oper1 + lit_name
                got_lit = True
                break
        if not got_lit:
            ...
            k = 'I'+str(i_ctr).rjust(3,'0')
            source_code_list.insert(len(source_code_list)-1, k+new_DC)
            original_source_code_dict[k] = new_DC
            i_ctr += 1
            lit_suffix = lit_suffix + 1
            lit_dict[lit_name] = lit
    ...
    if ("X'" in t or "B'" in t) and t.count(',') > 1 and mnemonic != 'DC':
        if "X'" in t:
            start = t.index("X'",0)
            end = t.index("'",start+2) + 1 
            field = t[start:end]
            v = str(int(field[2:-1], 16))
        elif "B'" in t:
            start = t.index("B'",0)
            end = t.index("'",start+2) + 1 
            field = t[start:end]
            v = str(int(field[2:-1], 2))
        source_code_list[sl_ctr] = sl_t + sl[0:15] + t[0:start] + v + t[end:]
    ...
    if mnemonic in assem_inst_list:
        ...
        if mnemonic in extended_mnemonic_inst_dict.keys():
            ...
            if mnemonic.endswith('R'):
                bc_mnemonic = 'BCR   '
            source_code_list[sl_ctr] = sl_t + sl[0:9] + bc_mnemonic + extended_mnemonic_inst_dict[mnemonic] + ',' + sl[15:]
            sl = source_code_list[sl_ctr][4:]
            sl_t = source_code_list[sl_ctr][0:4]
            mnemonic = sl[9:14].rstrip(' ')
...
while True:
    ...
    if sl[0] != '*' and (mnemonic in mach_inst_dict.keys() or (mnemonic == 'DC' or mnemonic == 'DS')):
        if (mnemonic == 'DS' and operand[0] == '0'):
            ...
            listing_line = hex(addr).lstrip('0x').rjust(6,'0').upper() + ' ' + 48*' ' + blp + original_source_code_dict[sl_t]
            if debug:
                print(listing_line)
            ...
            if debug:
                print(assem_output, (53-len(assem_output))*' ', blp+original_source_code_dict[sl_t])
            outputfi.write(assem_output + ' ' + (54-len(assem_output))*' ' + blp + original_source_code_dict[sl_t] + '\n')    
            aol_ctr = aol_ctr + 1
            instrdata.extend([code[i:i+2] for i in range(0,len(code),2) ])
            work_source_code_list.append(assem_output + ' ' + (54-len(assem_output))*' ' + ' ' + original_source_code_dict[sl_t])
    else:
        if (mnemonic == 'EQU' and operand[0] == '*') or mnemonic in [' DCB', ' OPEN', ' PUT', ' GET', ' CLOS', ' WTO']:
            (addr, code) = address_opcode_list[aol_ctr]
            listing_line = hex(addr).lstrip('0x').rjust(6,'0').upper() + ' ' + 48*' ' + blp + original_source_code_dict[sl_t]
            if debug:
                print(listing_line)
            ...
            if debug:
                print(display_addr, (53-len(display_addr))*' ', blp+original_source_code_dict[sl_t])
            outputfi.write(display_addr + ' ' + (54-len(display_addr))*' ' + blp + original_source_code_dict[sl_t] + '\n')
